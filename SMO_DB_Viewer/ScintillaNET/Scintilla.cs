	...
	public partial class Scintilla : System.Windows.Forms.Control, INativeScintilla, ISupportInitialize
	{
        ...
        protected override CreateParams CreateParams
        {
            ...
            get
            {
                ...
                if (!_sciLexerLoaded)
                {
                    IntPtr _lexer = new IntPtr(0);
                    // Look up GetSystemDirectory
                    System.Text.StringBuilder _sbSysDir = new System.Text.StringBuilder(255);
                    NativeMethods.GetSystemDirectory(_sbSysDir, _sbSysDir.Capacity);

                    if (NativeMethods.SetDllDirectory(@"c:\ScintillaNET\"))
                    { _lexer = NativeMethods.LoadLibrary(DefaultDllName); }
                    else
                    { // http://blogs.msdn.com/adam_nathan/archive/2003/04/25/56643.aspx
                        System.Diagnostics.Debug.WriteLine(new Win32Exception(Marshal.GetLastWin32Error()));
                    }

                    if (_lexer == IntPtr.Zero)
                    {
                        int errorCode = Marshal.GetLastWin32Error();
                        if (errorCode == NativeMethods.ERROR_MOD_NOT_FOUND)
                        {
                            // Couldn't find the SciLexer library. Provide a friendlier error message.
        ...
        {
            System.IO.FileInfo AppPath = new System.IO.FileInfo(Application.ExecutablePath);
            string ApplPath = AppPath.Directory.ToString();

            if (System.IO.Directory.Exists(ApplPath))
                return string.Concat(ApplPath);
            else
                return string.Empty;

            //int idx = ApplPath.IndexOf("bin");
            //if (idx > 0)
            //{
            //    string retPath = ApplPath.Substring(0, idx);
            //    return string.Concat(retPath, @"\");
            //}
            //else
            //{
            //    return ApplPath;
